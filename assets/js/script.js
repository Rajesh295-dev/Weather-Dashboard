...
var TargetedCity = document.querySelector("#cityTo-find");

var ForecastContainer = document.querySelector("#cityInformation");

//Array of Objects for localStores data
var dataStore = JSON.parse(localStorage.getItem('cities')) || [];

// check the UV index by latitude and longitude coordinates
var urlIcon;
...
var loadCity = function () {
    ...
    // To search the different city and added to the search history                  

    cleaningElement(SearchedCities);
    ...
    if (dataStore) {
        ...
        SearchedCities.appendChild(ulElement);
    }
...
var weatherHTML = function (city, uv) {
    ...
    cleaningElement(TargetedCity);
    cleaningElement(TargetedCity);

    // div for the City, date and weather condition
    ...
    TargetedCity.appendChild(ctn1);
    TargetedCity.appendChild(ctn2);
    TargetedCity.appendChild(ctn3);
    ...
    ctn7.innerHTML = "<h3>5-Day Forecast</h3>";
    ctn6.appendChild(ctn7);
    ForecastContainer.appendChild(ctn6);
    ...
    for (var i = 1; i < weatherCondition.length; i++) {
        // div and class from the boostrap 
...
// function to initial everything 
start();
